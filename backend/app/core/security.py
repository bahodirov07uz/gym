...
logger = logging.getLogger("auth")  # log nomi
...
def check_admin_role(
    ...
    # log qo‘shish
    logger.info(f"User ID: {user_id}, role from DB: {role_user.role if role_user else 'Not found'}")
    
    if not role_user or role_user.role.lower() != "admin":
        raise HTTPException(status_code=403, detail="Admin role required1")
